...
file_path = '/mnt/data/radar_measurements.csv'
...
# Read filtered values from CSV
filtered_values_file_path = '/mnt/data/FilteredValues_TrackId_31.csv'
filtered_values_csv = np.genfromtxt(filtered_values_file_path, delimiter=',', skip_header=1)
x, y, z = filtered_values_csv[:, 1], filtered_values_csv[:, 2], filtered_values_csv[:, 3]
cart2sph2(x, y, z, filtered_values_csv)
A = np.array([r, az, el])
...
for group_idx, group in enumerate(measurement_groups):
    for i, (x, y, z, mt) in enumerate(group):
        if not kalman_filter.first_rep_flag:
            ...
            if i == 1:
                # Initialize with the second measurement of the first group
                ...
                # Initialize with the first measurement of the next group if the first group has only one measurement
                prev_group_last_measurement = measurement_groups[group_idx - 1][-1]
                x_prev, y_prev, z_prev, mt_prev = prev_group_last_measurement
                dt = mt - mt_prev
                vx = (x - x_prev) / dt
                vy = (y - y_prev) / dt
                vz = (z - z_prev) / dt
            ...
            # Start filtering from the third measurement
            Z = np.array([[x], [y], [z]])
            if kalman_filter.gating(Z):
